...
{
    if (process_id < 0 || process_id >= OCULAROS_MAX_PROCESSES)
    {
        return NULL;
    }
...
void process_free(struct process* process, void* ptr)
{
    ...
    if (!allocation) // TODO: Kill the process
        return;
...
{
    int res = 0;
    paging_map_to(process->task->page_directory, (void*) OCULAROS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
int process_map_memory(struct process* process)
{
    ...
    paging_map_to(process->task->page_directory, (void*)OCULAROS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack+OCULAROS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
out:
    return res;
}
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(OCULAROS_USER_PROGRAM_STACK_SIZE);
