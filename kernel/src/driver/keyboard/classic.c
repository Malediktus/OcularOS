...
int classic_keyboard_init()
{
    ...
    keyboard_set_capslock(&classic_keyboard, KEYBOARD_CAPS_OFF);
...
uint8_t classic_keyboard_scancode_to_char(uint8_t scancode)
{
    ...
    if (keyboard_get_capslock(&classic_keyboard) == KEYBOARD_CAPS_OFF)
    {
        if (c >= 'A' && c <= 'Z')
        {
            c += 32;
...
void classic_keyboard_handle_interrupt()
{
    ...
    if (scancode == CLASSIC_KEYBOARD_CAPSLOCK)
    {
        ...
        keyboard_set_capslock(&classic_keyboard, old_state == KEYBOARD_CAPS_ON ? KEYBOARD_CAPS_OFF : KEYBOARD_CAPS_ON);
    }
